...
const GuessContainer: React.FC<GuessContainerProps> = ({ guessesData, animateIdx }) => {
  ...
  return (
    <div className="guess-container w-full max-w-full overflow-y-hidden overflow-x-auto px-1">
      {guessesData.length > 0 && (
        <>
          {/* Header Row */}
          <div className="guess-row flex mb-2 items-center justify-center min-w-[560px]">
            {columns.map((col) => (
              <div
                key={col}
                className="
          ...
          {guessesData.map((guess, idx) => {
            ...
            return (
              <div
                className={`guess-row flex items-center justify-center m-2 min-w-[560px] ${
                  shouldAnimate
                    ? rowVisible
                      ? "animate-slide-in"
                      : "opacity-0"
